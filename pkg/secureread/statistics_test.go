...
func TestStatisticsFromRecordsetKeepsTypedDateAndDecimalEvidence(t *testing.T) {
	...
	// Datetime buckets normalize chronology to a UTC calendar day.
	...
	if when.Types.Datetime != 1 || when.DateBuckets[0].Bucket != "2026-09-20" {
	...
	if invoiceDate.Types.Date != 1 || invoiceDate.Types.Datetime != 1 || len(invoiceDate.DateBuckets) != 2 || invoiceDate.DateBuckets[0] != (DateBucket{Bucket: "2013-12-22", Count: 1}) || invoiceDate.DateBuckets[1] != (DateBucket{Bucket: "2013-12-23", Count: 1}) {
