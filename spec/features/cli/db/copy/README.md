...
**First slice — schema-only.** The initial implementation replicates schema (collection definitions, primary keys, indexes) but NOT row data, because `dalgo2ingitdb` does not yet implement row CRUD (`Insert` / `RunReadwriteTransaction` / `ExecuteQuery*` are stubbed with `dal.ErrNotSupported`; tracked in [`docs/upstream-issues/ingitdb-cli-dalgo2ingitdb-row-crud.md`](../../../../../docs/upstream-issues/ingitdb-cli-dalgo2ingitdb-row-crud.md)). When the user invokes `db copy` against an inGitDB target, the command exits `0` after schema replication and emits a stderr note: `"row data not yet copied: dalgo2ingitdb has not implemented row CRUD (see <issue link>); rerun once it ships"`. Row streaming lights up once the upstream issue closes; the CLI contract does not change. PostgreSQL is recognized as a URL scheme but deferred from the MVP E2E bar until a PostgreSQL DALgo driver lands `dbschema.SchemaReader` + `ddl.SchemaModifier` + `dal.ConcurrencyAware`. The `ingitdb://` URL scheme dispatches to the inGitDB driver against a local-filesystem path.
