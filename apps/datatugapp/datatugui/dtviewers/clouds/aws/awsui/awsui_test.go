...
func TestViewerID(t *testing.T) {
	assert.Equal(t, dtviewers.ViewerID("aws"), viewerID)
}

func TestGoAwsHome_ReturnsNil(t *testing.T) {
	tui := newSafeTUI(t)
	err := goAwsHome(&AwsContext{TUI: tui}, sneatnav.FocusToMenu)
...
func TestRegisterAsViewer_ActionClosure(t *testing.T) {
	...
	// Invoke the Action closure — this covers lines 17-19 of home_ui.go.
	tui := newSafeTUI(t)
