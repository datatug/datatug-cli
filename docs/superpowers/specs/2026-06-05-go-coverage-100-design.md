...
**Execution model (refinement).** Because a workflow script cannot run `git`/`bash`,
the bash-bound phases run as **agents inside the workflow**, not in the main session:
- **Phase 0** worktree+branch creation = the git-orchestrator's first op.
- **Phase 1** = a **collector** agent (sonnet) that runs `go build`/`go test
  -coverprofile`, parses results, **writes `cover-skeleton.json` + baseline to the
  worktree**, and returns summary totals + quarantine + the skeleton path.
- **Phase 6** = an **overview** agent (sonnet) that re-runs total coverage and writes
  `TEST-COVERAGE-OVERVIEW.md`.
The workflow is therefore **fully self-contained**; the main session passes only
`args = { repoPath, stamp, pathFilter?, researchBudget?, engineerBudget?,
verifyRetries? }` (`stamp` supplied because `Date` is unavailable in scripts).

1. **Coverprofile parsing / skeleton shape.** Collector runs
   `go test <filter> -coverprofile=cover.out -covermode=atomic` + `go tool cover
   -func`, and parses `cover.out` blocks with `count==0` into uncovered line ranges.
   Skeleton (written to `cover-skeleton.json`, read by researchers/engineers):
   `{ packages: [{ importPath, dir, preCoveragePct, uncoveredLines, uncoveredFuncs,
   files: [{ file, functions: [{ name, uncoveredRanges: [[start,end],…] }] }] }],
   totals: { preCoveragePct, uncoveredLines } }`. To avoid huge JSON through schemas,
   the full skeleton lives on disk; agent returns carry only summaries + the path.

2. **StructuredOutput schemas.** Defined inline in the script for: collector,
   researcher (per-region `whyType`+`whyExplanation`+`approaches`, plus `newTypes`),
   synthesizer (helper plan, `usedByPackages` ≥3, `extendsExisting`), helper-builder,
   engineer (`postCoveragePct`, `uncoveredLinesRemaining`, `seamsAdded`,
   `documentedGaps`), verifier (`buildGreen`, `coveragePct`, `targetMet`,
   `meaningful`, `issues`), git-orchestrator (`status: ok|aborted`, `message`,
   `worktreePath`, `branch`), overview.

3. **Batch bin-packing.** Sort packages by import path (clusters same dir); greedily
   pack into batches until cumulative `uncoveredFuncs ≥ B` (default `B=25`); never
   split a package; a package with `>B` funcs is its own batch. Engineer **units**
   use the same greedy pack with budget `E` (default 20) within a top-level dir, kept
   disjoint so worktrees never overlap.

4. **`<stamp>`.** Generated by the main session and passed via `args.stamp`; used in
   `coverage/100-<stamp>` and worktree paths.

5. **git-orchestrator conflict escalation (MVP).** On any abort (conflict / non-clean
   / non-zero), the orchestrator runs `git merge --abort`, **leaves the worktree
   intact**, and reports `aborted`. The script records that unit as
   *integration-failed (manual merge required)* in the overview and does **not**
   retry — protecting the sync branch. (A dedicated sonnet `go.mod`/`go.sum` resolver
   is a deferred enhancement, not in the MVP.)
