...
- **Project-only export/import (no generic `db copy`).** Lost because it doesn't satisfy the user's stated "any DALgo source → any DALgo target" goal. SQLite↔PostgreSQL transfers would need a separate code path. Keeping `db copy` as the primitive and `project export/import` as sugar gives both ergonomics from one engine.
...
A focused spike landing `datatug db copy --from <url> --to <url>` with three E2E-tested backends (SQLite, inGitDB, PostgreSQL) and the minimum DALgo DDL extension required for auto-target-schema. `project export`/`project import` wrappers and configurable parallel streaming are stretch goals on top of the primitive. If the primitive ships and only one direction (e.g. SQLite→inGitDB) is wired for E2E, that is still a shippable MVP — the contract is set, additional backends fill in.
...
- **New Features this would create:**
  ...
  - `spec/features/cli/project/export/` and `spec/features/cli/project/import/` — thin wrappers that pin one endpoint to the current project (stretch within MVP).
